...
export const AuthorizedLayout = () => {
	const { isAuthorized, isInitialized } = useInsight(); // Read whether the user is authorized
	...
	const hasAccessKeys = !!(
		import.meta.env.CLIENT_ACCESS_KEY && 
		import.meta.env.CLIENT_SECRET_KEY
	);

	// wait for initialization before checking authorization to give access keys time to authenticate
	if (!isInitialized) {
		return <LoadingScreen />;
	}

	// After initialization, check authorization
	if (!isAuthorized) {
		// If access keys are present but still not authorized, something's wrong
		if (hasAccessKeys) {
			return (
				<div style={{ padding: '2rem' }}>
					<h2>Authentication Error</h2>
					<p>Access keys are configured but authentication failed.</p>
					<p>Please check your access keys in the .env.local file.</p>
				</div>
			);
		}
