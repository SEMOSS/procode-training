...
public class ReopenMcscCaseReactor extends AbstractTQMCReactor {

  private static final String REASON = "reason";
  private final LocalDateTime LOCAL_DATE = ConversionUtils.getUTCFromLocalNow();

  public ReopenMcscCaseReactor() {
  ...
  protected NounMetadata doExecute(Connection con) throws SQLException {
    ...
    // getting the latest mcsc case workflow block and the case to update reopen reason
    McscWorkflow reopenCase = TQMCHelper.getLatestMcscWorkflow(con, caseId);
    ...
    if (!assignee.getProducts().contains(TQMCConstants.MCSC)) {
      throw new TQMCException(
          ErrorCode.BAD_REQUEST, "Invalid request: assigned user no longer has MCSC Product");
    }
    ...
    try (PreparedStatement updateReason =
        con.prepareStatement(
            "UPDATE MCSC_CASE mc " + " SET mc.REOPENING_REASON = ? " + " WHERE mc.CASE_ID = ?")) {
    ...
    TQMCHelper.createNewMcscWorkflowEntry(con, reopenCase);
